...
const lctrim = (str) => (str === '' || str) ? (str).toLowerCase().trim() : undefined;
...
const hubspotSetContactToApplicationAssociation = (contactId, applicationId, data) => axios({
  method: "put",
...
const hubspotRecordBatchOperation = (path, operation) => async (data) => axios({
  method: "post",
...
const hubspotRecordBatchRead = (path) => hubspotRecordBatchOperation(path, OPERATION_READ);
const hubspotRecordBatchUpdate = (path) => hubspotRecordBatchOperation(path, OPERATION_UPDATE);
const hubspotRecordBatchCreate = (path) => hubspotRecordBatchOperation(path, OPERATION_CREATE);

const hubspotContactsBatchRead = async (data) => hubspotRecordBatchRead(CONTACTS_PATH)(data);
const hubspotContactsBatchUpdate = async (data) => hubspotRecordBatchUpdate(CONTACTS_PATH)(data);
const hubspotContactsBatchCreate = async (data) => hubspotRecordBatchCreate(CONTACTS_PATH)(data);

const hubspotApplicationsBatchRead = async (data) => hubspotRecordBatchRead(APPLICATIONS_PATH)(data);
const hubspotApplicationsBatchUpdate = async (data) => hubspotRecordBatchUpdate(APPLICATIONS_PATH)(data);
const hubspotApplicationsBatchCreate = async (data) => hubspotRecordBatchCreate(APPLICATIONS_PATH)(data);
    ...
    async.asyncify(async (list) => {
      try {
        ...
        const filteredContacts = list.filter((contact) =>
          contactIds.find(
            (existingContact) =>
              lctrim(existingContact.email) === lctrim(contact.email) ||
              lctrim(existingContact.additionalEmail) === lctrim(contact.email)
          )
        );

        const excludedContacts = list.filter((contact) =>
          !(contactIds.find(
            (existingContact) =>
              lctrim(existingContact.email) === lctrim(contact.email) ||
              lctrim(existingContact.additionalEmail) === lctrim(contact.email)
        ...
        const createResponse = await hubspotContactsBatchCreate(contactsToCreate);
...
*/
  ...
  console.log('All excluded emails: ', JSON.stringify(excludedContactsEmails, null, 2));
};
...
const updateAndCreateApplications = async (contacts) => {
  ...
  let n = 0;
	...
	console.log(`DEB total applications: ${applications.length} for ${contacts.length} contacts`); 
  ...
  await async.forEachLimit(
    ...
    async.asyncify(async (list) => {
      try {
	console.log(`DEB: batch[${batchIndex}], list.length =  "${list.length}", listToCreateOrUpdate="${listToCreateOrUpdate.length} "`);
//        console.log(`DEB: batch[${batchIndex}], list =  "${list.length}" "`, JSON.stringify(list, null, 2));

	batchIndex++;
        // Get applications id from unitid
        const getApplicationIdsResponse = await hubspotApplicationsBatchRead({
          idProperty: "unitid",
...
//	console.log(`DEV batch[batchIndex] data = `, JSON.stringify(data, null, 2))
          ...
          if (data.results.length > 0) {
            ...
            const filteredApplications = list.filter((app) =>
              applicationsIds.some(
                // DONE: change "==" => "==="
                (existingApplication) => String(existingApplication.unitid) === String(app.unitid)
              )
            );
            ...
            updateApplications.push(
              ...filteredApplications.map((app) => ({
                ...
                properties: {
                  ...
                  marketing_app_methods:
                    app.marketing_contact_methods,
                  stage: app.stage,
                  study_location_postcode: app.study_location_postcode,
            ...
            data.errors.forEach((error) => {
              if (error.category === "OBJECT_NOT_FOUND") {
                error.context.ids.forEach((id) => {
                  ...
                  // DONE: changed "==" => "==="
                  const applicationFromList = list.find((app) => String(app.unitid) === String(id));
                  ...
                  if (applicationFromList) {
                    createApplications.push({
                      properties: {
                        ...applicationFromList,
                        hs_pipeline_stage: helpers.stageToInternalId(applicationFromList.stage),
                      },
                    });
                  }
                });
        ...
        if (updateApplications.length > 0) {
          ...
          const updateApplicationResponse = await hubspotApplicationsBatchUpdate(transformedApplications);
        ...
        if (createApplications.length > 0) {
          ...
          const createApplicationResponse = await hubspotApplicationsBatchCreate(applicationsToCreate);
    ...
    async.asyncify(async (list) => {
      try {
        ...
        if (getContactIdsResponse.data.status === "COMPLETE") {
          contactsWithId.push(
            ...getContactIdsResponse.data.results.map((result) => ({
              ...
              email: result.properties.email,
              additionalEmail:
                lctrim(result.properties.hs_additional_emails),
            }))
          );
        }
        ...
        const updatedList = list.map((application) => {
          const applicationEmailLowerCaseTrim = lctrim(application.email);
          ...
          const contactWithId = contactsWithId.find(
            (c) =>
              ...
              lctrim(c.additionalEmail) === applicationEmailLowerCaseTrim
          );
          const contactId = contactWithId?.id;
          const applicationWithId = applicationsIds.find(
            // DONE: changed "==" => "==="
            (app) => String(app.unitid) === String(application.unitid)
          );
        ...
        for (const app of updatedList) {
          const association = await hubspotSetContactToApplicationAssociation(
            app.contactId,
            app.applicationId,
            JSON.stringify([
              {
